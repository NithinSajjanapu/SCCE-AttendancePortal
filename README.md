...
2. Use `npm install && npx puppeteer browsers install chrome` as the build command and `node server.js` as the start command. Puppeteer's committed backend configuration installs the required Chrome revision beside the deployed runtime dependencies rather than in Render's shared home cache.
