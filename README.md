...
2. Use `node scripts/repair-puppeteer-cache.cjs && npm install && npx puppeteer browsers install chrome` as the build command and `node server.js` as the start command. The repair step removes only a cached Linux Chrome directory that is missing its executable; a healthy cache is retained. Puppeteer's committed backend configuration installs the required Chrome revision beside the deployed runtime dependencies rather than in Render's shared home cache.
